public class day2problem2{
    public static void main(String[] args){
        ...
        for(int row = 0; row < input.length; row++){
            divisorsum += calcDivisorSum(input[row]);
            System.out.println("Current divisorsum: " + divisorsum);
        }
        System.out.println("Final divisorsum: " + divisorsum);
    }
    public static int calcDivisorSum(int[] row){
        for(int i = 0; i < row.length - 1; i++){
            for(int j = i+1; j < row.length; j++){
                System.out.println("row[i]="+ row[i] + " row[j]=" + row[j] + " modulo=" + row[i]%row[j]);
                if((row[i] % row[j]) == 0){
                    return (row[i] / row[j]);
